...
export default function NotesOtherFilter({
  ...
  return (
    <div
      className={`flex flex-col ${otherFeatureToggle ? "gap-2" : "gap-0"} transition-all duration-500`}
    >
        ...
        <div className="flex justify-between">
          <div className="flex gap-2 items-center">
            <input
              ...
              name="size"
              className="radio-box"
              checked={selectedOtherFilter === "featured"}
              onChange={() => onSelectedOtherFilter("featured")}
            />
            <label htmlFor="featured">Featured Notes</label>
          </div>
          <p className="notes-counter-bg">
            {data?.data?.stats?.featured_notes}
          </p>
        </div>
        <div className="flex justify-between">
          <div className="flex gap-2 items-center">
            <input
              ...
              name="size"
              className="radio-box"
              checked={selectedOtherFilter === "pinned"}
              onChange={() => onSelectedOtherFilter("pinned")}
            />
